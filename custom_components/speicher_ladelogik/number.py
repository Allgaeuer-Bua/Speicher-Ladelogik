...
NUMBERS = (
    SpeicherNumberDescription(key="bevorzugte_ladeleistung_venus_a", name="Bevorzugte Ladeleistung Venus A", control_key="bevorzugte_ladeleistung_a_w", minimum=0, maximum=5000, step=50, native_unit_of_measurement=UnitOfPower.WATT),
    SpeicherNumberDescription(key="bevorzugte_ladeleistung_venus_e", name="Bevorzugte Ladeleistung Venus E", control_key="bevorzugte_ladeleistung_e_w", minimum=0, maximum=5000, step=50, native_unit_of_measurement=UnitOfPower.WATT),
    ...
    SpeicherNumberDescription(key="manuell_laden_venus_a", name="Handbetrieb Laden Venus A", control_key="manuell_laden_a_w", minimum=0, maximum=5000, step=50, native_unit_of_measurement=UnitOfPower.WATT),
    SpeicherNumberDescription(key="manuell_entladen_venus_a", name="Handbetrieb Entladen Venus A", control_key="manuell_entladen_a_w", minimum=0, maximum=5000, step=50, native_unit_of_measurement=UnitOfPower.WATT),
    SpeicherNumberDescription(key="manuell_laden_venus_e", name="Handbetrieb Laden Venus E", control_key="manuell_laden_e_w", minimum=0, maximum=5000, step=50, native_unit_of_measurement=UnitOfPower.WATT),
    SpeicherNumberDescription(key="manuell_entladen_venus_e", name="Handbetrieb Entladen Venus E", control_key="manuell_entladen_e_w", minimum=0, maximum=5000, step=50, native_unit_of_measurement=UnitOfPower.WATT),
    ...
    SpeicherNumberDescription(key="venus_a_packs", name="Anzahl Packs Venus A", control_key="venus_a_packs", minimum=1, maximum=2, step=1),
)


async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddConfigEntryEntitiesCallback) -> None:
    coordinator: SpeicherLadelogikCoordinator = entry.runtime_data
    async_add_entities(SpeicherControlNumber(coordinator, description) for description in NUMBERS)
